...
df = sc.textFile(path).filter(
    lambda row: not row.startswith('@')).map(
        lambda row: [row.split()[x] if x < len(
            row.split()) else None for x in range(18)]).toDF(customSchema)


parseQname = udf(lambda x: x.split('_')[1])
parseCell = udf(lambda x: x.split('_')[2])

newDF = df.filter(col('XT').isNotNull()).withColumn(
...
readsDF.rdd.map(lambda x: "\t".join(map(str, x))).saveAsTextFile(output_path)
